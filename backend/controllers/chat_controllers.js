...
const normalizeWithGemini = async (text) => {
  try {
    const prompt = `Ubah kalimat berikut menjadi pertanyaan inti yang singkat, baku, dan jelas untuk keperluan pencarian FAQ,
tanpa mengubah maksud aslinya. Perbaiki typo dan singkatan (misal "gk" jadi "tidak", "gmn" jadi "bagaimana").
Jika membahas biaya/pembayaran, pertahankan kata kunci seperti biaya, bayar, tarif, harga, atau gratis.
Jawab HANYA dengan satu kalimat hasil perbaikan, tanpa tanda kutip, tanpa penjelasan apapun.
...
const generateNaturalAnswer = async (userQuestion, matchedFaq) => {
  try {
    const prompt = `Kamu adalah asisten FAQ. Tugasmu hanya merapikan jawaban referensi ini agar terdengar natural dan ramah,
tanpa mengubah fakta penting apa pun. Jangan menambah, mengurangi, atau membalik makna jawaban referensi.
Kalau ada kata seperti gratis, biaya, bayar, atau tidak ada pembahasan, pertahankan maknanya dengan tepat.
...
Jawab singkat dan jelas:`;
    ...
    return cleaned || stripLeadingGreeting(matchedFaq.jawaban);
  } catch (err) {
...
const gratitudeResponses = [
  "Sama-sama, senang bisa membantu. Kalau nanti ada yang ingin ditanyakan lagi, saya siap bantu ya.",
  "Sama-sama ya, semoga informasinya membantu. Kalau ada pertanyaan lain, boleh langsung tanya lagi.",
];
...
const isGratitudeText = (text) => {
  ...
  const gratitudePatterns = [
    /\bterima\s+kasih\b/, /\bmakasih\b/, /\bmakasih\s+banyak\b/, /\bthanks\b/, /\bthank\s*you\b/, /\bthankyou\b/, /\bthx\b/,
  ];
...
const acknowledgementResponses = [
  "Baik, senang bisa membantu. Kalau sudah tidak ada lagi, semoga harimu lancar ya.",
  "Siap, kalau begitu. Terima kasih sudah bertanya, dan kalau butuh bantuan lagi saya ada di sini.",
];
...
const isAcknowledgementText = (text) => {
  ...
  const ackPatterns = [
    ...
    /^(ng?gak|ngga|enggak)\s*(ada|apa)?\b/, /\b(?:ga|gak|nggak|ngga|enggak)\s*ada\s*(?:lagi)?\b/, /\bgaada\b/,
    /\bsuksma\b/, /\bmakasi+h*\b/,
  ];
    ...
    try {
      ...
      if (bestMatch && highestScore >= THRESHOLD) {
        ...
        return res.json({
          status: "not_found",
          data: {
            jawaban: "Maaf, informasinya belum tersedia saat ini. Kalau nanti ada yang ingin ditanyakan lagi, saya siap bantu dengan senang hati.",
          },
        });
      }
    } catch (err) {
